...
#burning in period
fit0 <- StartNewsamples(dmi, priors, ncore = 4, nmc=1e3)  # 2e3


##fit the model = the actuall running ####
fit <- run(fit0, 8e3, ncore = 4, thin = 12) # 6e3
...
write.csv(GR_dr,file="GelmanRubin_DR_all.csv")
write.csv(dr_sum,file="summary_DR_all.csv")


write.csv(GR_dr,'C:/Users/elado/Desktop/LBA_analysis_positive - Copy/GelmanRubin_DR_all.csv',row.names = FALSE)
write.csv(dr_sum,"C:/Users/elado/Desktop/LBA_analysis_positive - Copy/summary_DR_all.csv",row.names = FALSE)
